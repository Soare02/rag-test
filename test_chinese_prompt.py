...
def calculate_score(logprobs, full_text, yes_tokens, no_tokens):
    if not logprobs:
        return 0.0
    ...
    for idx, token_info in enumerate(logprobs):
        ...
        if token_str in yes_tokens or token_str in no_tokens:
            ...
            # Print top 5 choices
            print("  Top choices:")
            for c in top_logprobs[:5]:
                print(f"    - {repr(c['token'])}: {c['logprob']}")
            ...
            print(f"  Max yes_lp: {yes_lp}, Max no_lp: {no_lp}")
...
test_cases = [
    {
        "name": "Chinese prompt output yes/no",
        "system": "你是一个文档相关性判定助手。你必须只回答 'yes' 或 'no'，不要输出任何解释或前缀。",
        "user": "用户问题：{query}\n参考文档：{doc}\n该文档是否相关，能回答该问题吗？请仅回答 'yes' 或 'no'。是否相关:",
        "yes_tokens": {"yes", "Yes", "YES"},
        "no_tokens": {"no", "No", "NO"}
    },
    {
        "name": "Chinese prompt output 是/否",
        "system": "你是一个文档相关性判定助手。你必须只回答 '是' 或 '否'，不要输出任何解释或前缀。",
        "user": "用户问题：{query}\n参考文档：{doc}\n该文档是否相关，能回答该问题吗？请仅回答 '是' 或 '否'。是否相关:",
        "yes_tokens": {"是", "Yes", "yes"},
        "no_tokens": {"否", "No", "no"}
    }
]
...
for case in test_cases:
    ...
    for doc_name, doc_text in [("Doc 1 (Relevant)", doc1), ("Doc 2 (Irrelevant)", doc2)]:
        print(f"\n--- {doc_name} ---")
        user_content = case["user"].format(query=query, doc=doc_text)
        payload = {
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                score = calculate_score(logprobs, full_text, case["yes_tokens"], case["no_tokens"])
